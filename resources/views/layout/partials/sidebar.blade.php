        ...
        <ul>
            ...
            @foreach ($menus as $menuItem)
                    ...
                    @if ($menuItem->subscribe == 1 && Auth::user()->isSubscribe == 1)
                        <li>
                            <a class="{{ Request::is($menuItem->menu->url) ? 'active' : '' }}"
                                href="{{ url($menuItem->menu->url) }}"><i class="{{ $menuItem->menu->icon }}"></i><span>
                ...
                @else
                    ...
                    @if ($menuItem->subscribe == 1 && Auth::user()->isSubscribe == 1)
                        <li class="submenu">
                            <a class="{{ Request::is(...$subMenuItem) ? 'active' : '' }}" href="javascript:void(0);"><i
                                    class="{{ $menuItem->menu->icon }}"></i><span>
                                    {{ Str::title($menuItem->menu->name) }}</span> <span class="menu-arrow"></span></a>
                            <ul>
                                @foreach ($menuItem->menu->subMenu as $subMenuItems)
