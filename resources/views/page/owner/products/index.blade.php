                    ...
                    <div class="row">
                        <div class="col-12 col-lg-3 col-md-3">
                            <div class="form-group" id="categoryField">
                                <select name="category" id="category" class="select @error('category') is-invalid @enderror">
                                    ...
                                    <option value="{{ $item->code }}" {{ request()->get('category') == $item->code ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-lg-3 col-md-3">
                            <div class="form-group" id="brandField">
                                <select name="brand" id="brand" class="select @error('brand') is-invalid @enderror">
                                    <option value="null" selected disabled>Filter Merek</option>
                                    <option value="all" {{ request()->get('brand') == 'all' ? 'selected' : '' }}>
                                        Semua Merek</option>
                                    @foreach ($brands as $item)
                                    <option value="{{ $item->id }}" {{ request()->get('brand') == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-12 col-lg-3 col-md-3 offset-lg-3 offset-md-3">
                            <div class="form-group d-flex align-items-center gap-3">
                                <a class="btn btn-filters ms-auto" id="filter"><img src="{{ URL::asset('assets/img/icons/search-whites.svg') }}" alt="img" data-bs-toggle="tooltip" title="Filter"></a>
                                <a class="btn btn-filters" id="resetFilter"><i class="fa fa-undo" data-bs-toggle="tooltip" title="Reset Filter"></i></a>
                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="PDF" id="reportPdf"><img src="{{ URL::asset('assets/img/icons/pdf.svg') }}" alt="img"></a>
                                <a data-bs-toggle="tooltip" data-bs-placement="top" title="Excel" id="reportExcel"><img src="{{ URL::asset('assets/img/icons/excel.svg') }}" alt="img"></a>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr>
                                    <td class="productimgname">
                                        <a href="javascript:void(0);" class="product-img">
                                            ...
                                            <img src="{{env("AWS_ENDPOINT_BUCKET")}}/{{  $product->images }}" alt="{{ $product->name }}" />
                                            @elseif ($product->images == 'noimage.png')
                                            <img src="{{ URL::asset('images/' . $product->images) }}" alt="{{ $product->name }}" />
                                            @endif
                                        </a>
                                        {{ $product->name }}
                                    </td>
                                    ...
                                    <td>
                                        <a class="me-3" href="{{ route('owner.produk.daftar-produk.edit', ['id' => Crypt::encrypt($product->id)]) }}">
                                            <img src="{{ URL::asset('assets/img/icons/edit.svg') }}" alt="img" />
                                        </a>
                                        <a class="me-3" id="confirm-delete" data-action="{{ route('owner.produk.daftar-produk.delete', ['id' => Crypt::encrypt($product->id)]) }}">
